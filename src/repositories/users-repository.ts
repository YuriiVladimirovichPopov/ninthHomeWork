...
export const usersRepository = {
  ...
  async findAllUsers(
    ...
    const result: UsersMongoDbType[] = await usersCollection
      .find(filter, { projection: { passwordSalt: 0, passwordHash: 0 } })
  ...
  async deleteUser(id: string): Promise<boolean> {
    ...
    const foundUserById = await usersCollection.deleteOne({
      _id: new ObjectId(id),
    });
